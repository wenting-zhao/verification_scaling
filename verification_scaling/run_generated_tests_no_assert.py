...
from verification_scaling.utils import extract_code
...
def main():
    ...
    for code, tests in zip(generated_code, generated_tests):
        tests['test_cases'] = [test.replace("assert ", "").split("==")[0].strip() for test in tests['test_cases']]
    ...
    output_dataset_name = f"i_code_{code_dataset_name}_tests_{test_dataset_name}"
